        ...
        void try_infer_width(void)
            {
                ...
                case opcode::LSH:
                {
                    /* Here's an odd case: I'm defining LSH to do an
                     * implicit zero-extend or truncate, just like RSH
                     * does.  Note that this will only happen if
                     * there's an explicit width set inside the Flo
                     * file, otherwise this will get inferred
                     * correctly below and then this will get hit
                     * later. */
                    if (_width.known()) {
