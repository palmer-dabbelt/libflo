...
namespace libflo {
    ...
    public:
        ...
        std::vector<std::shared_ptr<node_t>> operands(void) const
            {
        ...
        std::vector<std::shared_ptr<node_t>> sources(void) const
            {
                return _s;
            }
        ...
        void try_infer_width(void)
            {
                ...
                case opcode::LSH:
                {
                    ...
                    /* FIMXE: Here's a special case for the assembly
                     * generated by DREAMER. */
                    if (globals::allow_bad_widths
                        && _width.known()
                        && d()->known_width()
                        && (_width.value() != d()->width())) {
                        /* Set the width to the operation's width, not
                         * the node's width.  The logic behind this is
                         * that Albert has written a Chisel operator
                         * that's explicitly specified the width, so
                         * we should use that instead of anything else
                         * that may be floating around. */
                        size_t real_width = d()->width();
                        must_be(0, _width.value());
                        d()->update_width(real_width);
                        _width = real_width;
                    }
